...
int sprites_per_view=1;
    ...
    if(flags & CAR_IS_SPINNING) sprites_per_view=32;
...
void project_render_sprites(project_t* project, object_t* object)
{
    ...
    for (i = 0; i < NUM_CARS; i++) {
        ...
        if (sprite_flags & SPRITE_RESTRAINT_ANIMATION) {
            render_loading(images, animation, flags, base_frame, sprites_per_image,
                car_images);
...
object_t* project_export_dat(project_t* project)
{
    ...
    object->ride_header->categories[1] = (project->flags & RIDE_WET || project->flags & RIDE_SLOW_IN_WATER) ? CATEGORY_WATER_RIDE : 0xFF;
    ...
    for (i = 0; i < NUM_CARS; i++) {
        if (cars_used[i] || project->cars[i].flags & CAR_CAN_INVERT) {
            ...
            if (object->ride_header->categories[0] == CATEGORY_ROLLERCOASTER){
                object->ride_header->cars[i].flags |= CAR_RIDERS_SCREAM;
            }
            /*
            switch (i) {
                case 0:
                    object->ride_header->cars[i].effect_visual = 0x0Au;
                    break;
                default:
        ...
        // printf("flags %x\n",project->cars[i].flags);
            ...
            if (project->cars[i].flags & CAR_IS_SWINGING) {
                ...
                FLAG_21 | FLAG_25 | FLAG_27 | number of sprites
                    ...
                    1         1         1       13
                */
                object->ride_header->cars[i].flags |= CAR_FLAG_21;
                object->ride_header->cars[i].extra_swing_frames = 0x08u; //this is 1<<27 (enables 13 frames instead of 7)
            }
            ...
            if (project->cars[i].flags & CAR_IS_POWERED) {
                object->ride_header->cars[i].powered_velocity = 6;
                object->ride_header->cars[i].powered_acceleration = 0x80u;
            }
            if (project->cars[i].flags & CAR_IS_SPINNING) {
                object->ride_header->cars[i].spin_inertia = 40;
