...
float try_catch_float(json_t* json, char* key, float def)
{
    json_t* deserialize = json_object_get(json, key);
    if (deserialize != NULL)
    {
        return json_real_value(deserialize);
    }
    else
    {
        return def;
...
json_t* project_serialize(project_t* project)
{
    ...
    for (i = 0; i < NUM_CARS; i++) {
        ...
        if (cars_used[i] || project->cars[i].flags & CAR_CAN_INVERT) {
            ...
            json_t* animation_speed_modifier = json_real(project->cars[i].animation_speed_modifier);
            json_object_set_new(car,"animation_speed_modifier",animation_speed_modifier);
            
            json_t* steam_effect_modifier_X = json_real(project->cars[i].steam_effect_modifier[0]);
            json_object_set_new(car,"steam_effect_modifier_X",steam_effect_modifier_X);
            json_t* steam_effect_modifier_Z = json_real(project->cars[i].steam_effect_modifier[1]);
            json_object_set_new(car,"steam_effect_modifier_Z",steam_effect_modifier_Z);
            printf("saving datas %f %f %f\n",project->cars[i].animation_speed_modifier,project->cars[i].steam_effect_modifier[0],project->cars[i].steam_effect_modifier[1]);
        } else
            car = json_null();
...
project_t* project_deserialize(json_t* json)
{
    ...
    for (unsigned int i = 0; i < NUM_CARS; i++) {
        ...
        if (!json_is_null(car)) {
            ...
            project->cars[i].animation_speed_modifier = try_catch_float(car,"animation_speed_modifier",1);
            project->cars[i].steam_effect_modifier[0] = try_catch_float(car,"steam_effect_modifier_X",1);
            project->cars[i].steam_effect_modifier[1] = try_catch_float(car,"steam_effect_modifier_Z",1);
            printf("loading datas %f %f %f\n",project->cars[i].animation_speed_modifier,project->cars[i].steam_effect_modifier[0],project->cars[i].steam_effect_modifier[1]);
        }
    }
    return project;
}
