...
uint32_t calculate_checksum(uint8_t* header, uint8_t* data, uint32_t size)
{
    ...
    for (i = 4; i < 12; i++)
        checksum = checksum_process_byte(checksum, header[i]); /*Do filename*/
    for (i = 0; i < size; i++)
        checksum = checksum_process_byte(checksum, data[i]); /*Checksum rest file*/
    ;
    return checksum;
}
...
buffer_t* decompress_data(uint8_t* encoded_bytes, uint32_t size)
{
    ...
    uint32_t pos = 0;
    while (pos < size) {
        /*encoded_bytes is left unsigned for the sake of consistency, so test
     * negativity by looking at the high order bit*/
...
buffer_t* compress_data(uint8_t* decoded_bytes, uint32_t size)
{
    ...
    while (pos < size) {
        ...
        if (repeated_bytes > 1) {
            ...
            buffer_write(encoded_bytes, &byte_to_wryte,
                1); /*Number of bytes to repeat*/
            ...
            buffer_write(encoded_bytes, decoded_bytes + pos,
                bytes_to_copy); /*Byte to copy*/
            pos += bytes_to_copy;
        }
    }
...
ride_header_t* ride_header_load(uint8_t* bytes, uint32_t* pos_ptr)
{
    ...
    for (i = 0; i < NUM_CARS; i++) {
        ...
        /*Read unknown fields*/
        car->car_effect = car_data[93];
        car->effect_visual = car_data[94];;
    ...
    *((uint64_t*)(header_bytes + 438)) = header->track_sections;
    ...
    for (i = 0; i < NUM_CARS; i++) {
        ...
        *((uint32_t*)(car_data + 17)) = car->flags;
        /*Write "extra frames"*/
        car_data[21] = car->extra_swing_frames; //this should be a flag!!!
        ...
        car_data[93] = car->car_effect;
        car_data[94] = car->effect_visual;
...
ride_structures_t* ride_structures_load(uint8_t* bytes, uint32_t* pos_ptr)
{
    ...
    int pos = *pos_ptr;
    ride_structures_t* structures = malloc(sizeof(ride_structures_t));
    /*Sequence of 3 byte structures representing the default color schemes; I
   * don't know why -1 means 32*/
    ...
    for (i = 0; i < 4; i++) {
        ...
        pos++;
        /*If the length can't fit in a byte, the length is instead stored in the two
     * bytes following*/
        if (len == 0xFF) {
...
void ride_structures_write(ride_structures_t* structures, buffer_t* buffer)
{
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (structures->peep_positions[i].num < 0xFFu)
            buffer_write(buffer, (uint8_t*)&structures->peep_positions[i].num, 1);
        ...
        /*Write structure*/
        buffer_write(buffer, structures->peep_positions[i].positions,
...
void ride_structures_free(ride_structures_t* structures)
{
    int i;
    for (i = 0; i < 4; i++)
        free(structures->peep_positions[i].positions);
    free(structures);
}
...
void string_table_write(string_table_t* table, buffer_t* buffer)
{
    ...
    for (i = 0; i < table->num_strings; i++) {
        ...
        buffer_write(buffer, (uint8_t*)table->strings[i].str,
            strlen(table->strings[i].str) + 1);
    }
...
image_list_t* image_list_load(uint8_t* bytes, uint32_t* pos_ptr)
{
    ...
    for (i = 0; i < list->num_images; i++) {
        ...
        {
            for (j = 0; j < image->height; j++) {
                /*The data starts with a list of two byte offsets to the individual row
         * data.*/
                ...
                do {
                    ...
                    /*The most significant bit of the length is set only if this is the
           * last element*/
...
void image_list_write(image_list_t* list, buffer_t* buffer)
{
    ...
    for (i = 0; i < list->num_images; i++) {
        ...
        *((uint16_t*)(image_record_ptr + 14)) = 0;
        ...
        if (image->flags == 1) {
            for (j = 0; j < image->height; j++)
                buffer_write(buffer, image->data[j], image->width);
        } else /*image->flags==5*/
        {
            /*Store offset of the list of row offsets, and leave space for them in the
       * buffer*/
            ...
            for (j = 0; j < image->height; j++) {
                ...
                *((uint16_t*)(buffer->data + row_offset_list_offset + j * 2)) = buffer->size - row_offset_list_offset;
                ...
                while (x_displacement < image->width && image->data[j][x_displacement] == 0)
                    x_displacement++;

                do {
                    length = 0;
                    /*Find length. Sections can't be longer that 126 for some reason*/
                    while (x_displacement + length < image->width && image->data[j][x_displacement + length] != 0 && length < 126)
                        length++;
                    /*Compute new x_displacement*/
                    uint8_t new_x_displacement = x_displacement + length;
                    while (new_x_displacement < image->width && image->data[j][new_x_displacement] == 0)
                        new_x_displacement++;
                    /*If there are no more non transparent pixels, set last one flat*/
                    if (new_x_displacement == image->width)
                        length |= 0x80;
                    /*Write length*/
...
object_t* object_load_dat(const char* filename)
{
    ...
    /*Check the header to see if the remaining data is compressed- if so, then
   * decode it*/
    buffer_t* bytes;
    if (header[0x10]) {
...
void object_save_dat(object_t* object,
    const char* filename,
    uint32_t checksum)
{
    int i;
    ...
    while (filename[start_index] != '/' && filename[start_index] != '\\')
        start_index--;
    start_index++;
    while (filename[end_index] != '.' && end_index > start_index)
        end_index--;
    /*If there's no extension, set end_index to the end of the file*/
    if (start_index == end_index)
        end_index = filename_length - 1;
    else
        end_index--;
    ...
    /*Calculate checksum - commented out because checksum is now passed as a
   * parameter*/
    // uint32_t
    // checksum=calculate_checksum(header,decoded_file->data,decoded_file->size);
    /*Write checksum*/
